...
ONLY_IN_CONF = False                       # Если Истина, то не регистрируются ошибки во внешних обработках и внешних отчетах
USE_WHOIS = False                           # Если Истина, то по IP адресу будет запрос к WHOIS на опеределие домена и организации

# Черные и белые списки. Списко ip адресов, где адреса без ведущих нулей. Допускается неполное задание, например, '192.168.'
BLACKLIST = ['127.0.0.1']
WHITELIST = []
...
MIN_REPORTS = 1   # число отчетов от разных юзеров после которого ошибка попадает на рассмотрение