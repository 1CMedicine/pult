...
def prepareErrorTable(cur, output, errorN = 0):
    ...
    for r in cur.fetchall():
        ...
        if errorN == 0:
            print("<td align='center'><span class='errorId'><a href='", prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, "/reports/",str(r[0]),"'>",str(r[0]),"</a></span></td>", sep='', end='', file=output)
    ...
    try:
        ...
        for configName in errors:
            ...
            for r in errors[configName]:
                print('   ', prefs.SITE_DOMAIN, prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, "/reports/", r, sep='', end='\n', file=output)
...
.refli  {
    ...
    http.open('POST', "''',  prefs.SITE_URL,  prefs.RESTICTED_AREA_PREFIX, '''/markError/"+error, true);
...
function selectConfig(configName) {
    if (configName != 'sn')
        document.location.href="''', prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, '''/errorsList/"+configName.substring(1)
    else
        document.location.href="''', prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, '''/errorsList"
}''', sep='', file=output)
    ...
    if environ['PATH_INFO'] == prefs.RESTICTED_AREA_PREFIX+'/settings':
        output = StringIO()
        print('''<html><head>
<meta charset='utf-8'>
...
<p>Если список пустой, то почта для этой конфигурации не отправляется.</p>''', sep='', file=output)
        ...
        print("<ul><li class='refli'><a href='", prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, "/errorsList'>Список ошибок</a></lu>", sep='', file=output)
        print("<li class='refli'><a href='", prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, "/clear'>Удаление отчетов неподдерживаемых версий и конфигураций</a></li>", sep='', file=output)
    ...
    if environ['PATH_INFO'] == prefs.RESTICTED_AREA_PREFIX+'/clear':
        output = StringIO()
        ...
        print("<hr><h3>Перейти:</h3><p><a href='", prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, "/errorsList'>Список ошибок</a></p>", sep='', file=output)
    ...
    if len(prefs.RESTICTED_AREA_PREFIX) > 0:
        url.pop(0)
...
<p><a href="''', prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, '''/settings">Настройки сервиса</a></p>
...
<th>Число отчетов</th></tr>''', sep='', file=output)
        ...
        cur.execute(SQLPacket)
        for r in cur.fetchall():
            print("<tr><td><span class='descTime'>", r[0], "</span></td><td>", r[1], "</td><td>", r[13], "</td><td>", r[2], "</td><td>",r[3],"</td><td>", r[4],"</td><td>",r[5],"</td><td>", r[6],"</td><td align='center'>",r[10],"</td><td>","" if r[12] is None else r[12],"</td><td align='center'>","<a href='",prefs.SITE_URL,prefs.RESTICTED_AREA_PREFIX,"/report/",r[9],"'>",'Файл(ы)' if r[14]==1 else r[8],"</a></td></tr>", sep='', file=output)

        cur.close()
        conn.close()
        print("</table>", sep='', file=output)
        print('''<p><a href='https://its.1c.ru/db/v8320doc#bookmark:dev:TI000002262'>Документация на ИТС по отчету об ошибке</a></p>
<p><a href="''', prefs.SITE_URL, prefs.RESTICTED_AREA_PREFIX, '''/errorsList">Список ошибок</a></p>''', sep='', file=output)
