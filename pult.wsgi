...
def readReport(fzip_name):
    tdir = tempfile.TemporaryDirectory()
    with zipfile.ZipFile(fzip_name, 'r') as zip_ref:
...
.refli  {
    ...
    if environ['PATH_INFO'] == '/settings':
        ...
        print("<p>Отчет считается новой ошибкой, если образуется уникальная кобинация из следующих данных отчетов:</p>", sep='', file=output)
        print("<ul><li class='refli'>Наименования конфигурации</li>", sep='', file=output)
        print("<li class='refli'>Версии конфигурации</li>", sep='', file=output)
        print("<li class='refli'>Установленных ресширений</li>", sep='', file=output)
        print("<li class='refli'>Текст ошибки</li>", sep='', file=output)
        print("<li class='refli'>Хеша стека конфигурации ошибки</li></ul>", sep='', file=output)
...
<th>Число отчетов</th></tr>''', sep='', file=output)
        for r in cur.fetchall():
            print("<tr><td><span class='descTime'>", r[0], "</span></td><td>", r[1], "</td><td>", r[13], "</td><td>", r[2], "</td><td>",r[3],"</td><td>", r[4],"</td><td>",r[5],"</td><td>", r[6],"</td><td align='center'>",r[10],"</td><td>",r[12],"</td><td align='center'>","<a href='",prefs.SITE_URL,"/report/",r[9],"'>",r[8],"</a></td></tr>", sep='', file=output)
